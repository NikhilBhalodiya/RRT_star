...
//    subscribing the goal topic
    m_goal_sub = nh.subscribe<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10, &RRTStar::GoalPoseCallback, this);
//publiser defination for new map on which expploration can be visualize
    m_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("/map_new",10);
//  publisher defination of final goal with green line on map
    m_path_pub = nh.advertise<nav_msgs::Path>("/global_path", 10);
//  getting initial parameters eg. start position in the map
    getStartParam();
//taking tiime to seed  random generation
...
{
//drawing initial start and goal position                                                    //ROS_INFO("Path planning has been started");
  drawStartAndGoal();
...
{
    ROS_INFO("Start and Goal has been Initiated");
...
//    double rand_pose_y = double(std::rand() % 3.0) + 7.0;
    ...
    //generating random points only in the area of near by white spcaes .  . actuallly map image is big but free space is only 387*387 pixel
    rand_pose_x = Randomdouble(7,11.7);   // values Found by try and error from the map It  is specific to this map only
...
void RRTStar::expandNearestNode()
{
   ...
   if(isInFreeSpace(newNode))                                             // m_map_new.data[(m_map->info.width*new_node_grid_y + new_node_grid_x)] == 0)
   {
       ...
       getBestParent();
        node_count++;
        ROS_INFO("Node count is %d ", (node_count));
        int count = 0;
        int size = int(node_in_radius.size());
...
{
    return atan2(a.y_coordinate - b.y_coordinate ,a.x_coordinate - b.x_coordinate);
}


void RRTStar::createNewNode()
{
//    Expanntion of the nearest node is in the direction of random node with a fixed step size
...
void RRTStar::reWireTheTree()
{
    for(auto &check_node : node_in_radius)
    {
        ...
        {     ROS_INFO("goal node in radius");
            if(check_node.node_id != goal.node_id)
            {
...
void RRTStar::getNeighboursInKRadius()
{
    ...
    radius =std::pow((std::log(node_count)/node_count),(1.0/dimention_of_spcae));
   ...
   if(m_have_map && m_have_current_pose)
    {
       ...
       {
         ROS_INFO("more random points needs to be generated");
